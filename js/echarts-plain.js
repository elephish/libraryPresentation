...
(function (e) {
    ...
    (function (e) {
        ...
        if (typeof window == "undefined" || window !== e) {
            self.tmd = {}, self.tmd.modules = {}, self.tmd.require = function (e) {
                return self.tmd.modules[e]
            }, self.define = function (e, t, n) {
                if (arguments.length === 0)return;
                arguments.length == 1 ? (n = e, e = "ForceLayout") : arguments.length == 2 && (n = t), e.indexOf("ForceLayout") >= 0 && (e = "ForceLayout"), self.tmd.modules[e] = n(self.tmd.require)
            ...
            self.onmessage = function (e) {
                if (e.data instanceof ArrayBuffer) {
                    if (!t)return;
                    var n = new Float32Array(e.data), r = (n.length - 1) / 2;
                    for (var i = 0; i < r; i++) {
                        var s = t.nodes[i];
                        s.position[0] = n[i * 2 + 1], s.position[1] = n[i * 2 + 2]
                    }
                    return
                }
                var o = self.tmd.modules.ForceLayout;
                switch (e.data.cmd) {
                    case"init":
                        t || (t = new o), t.initNodes(e.data.nodesPosition, e.data.nodesMass, e.data.nodesSize), t.initEdges(e.data.edges, e.data.edgesWeight), t._token = e.data.token;
                        break;
                    case"updateConfig":
                        if (t)for (var u in e.data.config)t[u] = e.data.config[u];
                        break;
                    case"update":
                        var a = e.data.steps;
                        if (t) {
                            var r = t.nodes.length, n = new Float32Array(r * 2 + 1);
                            t.temperature = e.data.temperature;
                            if (e.data.temperature > .01) {
                                for (var i = 0; i < a; i++)t.update(), t.temperature *= e.data.coolDown;
                                for (var i = 0; i < r; i++) {
                                    var s = t.nodes[i];
                                    n[i * 2 + 1] = s.position[0], n[i * 2 + 2] = s.position[1]
                                }
                                n[0] = t._token
                            }
                            self.postMessage(n.buffer, [n.buffer])
                        } else {
                            var f = new Float32Array;
                            self.postMessage(f.buffer, [f.buffer])
                        }
                }
            }
        }
        ...
        return g.prototype = {constructor: g, type: o.CHART_TYPE_FORCE, _init: function () {
            ...
            var u = this.parseCenter(this.zr, e.center), f = this.parsePercent(e.size, this.zr.getWidth()), p = this.parsePercent(e.size, this.zr.getHeight()), d = Math.min(f, p), v = [];
            for (var g = 0; g < i; g++) {
                var y = t[g];
                v.push(y.value || 1)
            }
            ...
            var w = b.max();
            if (w === 0)return;
            var S = b.mul(1 / w, b).toArray(), x = new h(i * 2);
            for (var g = 0; g < i; g++) {
                var T, y = t[g];
                typeof this.__nodePositionMap[y.name] != "undefined" ? (T = l.create(), l.copy(T, this.__nodePositionMap[y.name])) : typeof y.initial != "undefined" ? T = Array.prototype.slice.call(y.initial) : T = E(u[0], u[1], d * .8);
                var N = r[g].style;
                N.width = N.width || v[g] * 2, N.height = N.height || v[g] * 2, N.x = -N.width / 2, N.y = -N.height / 2, r[g].position = T, x[g * 2] = T[0], x[g * 2 + 1] = T[1]
            }
            i = n.length;
            var C = new h(i * 2), k = new h(i);
            for (var g = 0; g < i; g++) {
                var L = n[g];
                C[g * 2] = L.source, C[g * 2 + 1] = L.target, k[g] = L.weight || 1
            }
            b = new c(k);
            var w = b.max();
            if (w === 0)return;
            var k = b.mul(1 / w, b)._array, A = {center: u, width: e.ratioScaling ? f : d, height: e.ratioScaling ? p : d, scaling: e.scaling || 1, gravity: e.gravity || 1, barnesHutOptimize: e.large};
            this._layoutWorker ? (this._token = m(), this._layoutWorker.postMessage({cmd: "init", nodesPosition: x, nodesMass: S, nodesSize: v, edges: C, edgesWeight: k, token: this._token}), this._layoutWorker.postMessage({cmd: "updateConfig", config: A})) : (a.merge(this._layout, A, !0), this._layout.initNodes(x, S, v), this._layout.initEdges(C, k))
